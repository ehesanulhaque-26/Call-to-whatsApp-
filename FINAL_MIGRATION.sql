...
-- refresh_tokens table MUST be created before any reference
...
CREATE POLICY "Users can view their own profile" ON users FOR SELECT USING (auth.uid() = id AND deleted_at IS NULL);
CREATE POLICY "Users can update their own profile" ON users FOR UPDATE USING (auth.uid() = id AND deleted_at IS NULL);
CREATE POLICY "Admins can view all users" ON users FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin' AND deleted_at IS NULL));
CREATE POLICY "Admins can update all users" ON users FOR UPDATE USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin' AND deleted_at IS NULL));
...
CREATE POLICY "Admins can view all activity logs" ON activity_logs FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin' AND deleted_at IS NULL));
...
CREATE POLICY "Users can view their own refresh tokens" ON refresh_tokens FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can insert their own refresh tokens" ON refresh_tokens FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own refresh tokens" ON refresh_tokens FOR UPDATE USING (user_id = auth.uid());
